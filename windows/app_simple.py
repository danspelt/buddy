...
# Simple VAD replacement - just check audio level
def is_speech_frame(pcm16, threshold=1000):
    """Simple voice activity detection based on audio level"""
    return np.max(np.abs(pcm16)) > threshold
...
REQUIRE_WAKE = cfg.getboolean("wake", "require_wake_word", fallback=False)  # Disable for now
...
def heard_wake(pcm16):
    if not REQUIRE_WAKE:
        return True
    # Simple wake word detection - just check if audio level is high enough
    return np.max(np.abs(pcm16)) > 2000
...
async def main_loop():
    ...
    stream.start()
    print("Buddy Windows listener started. Say 'brighton' when you're ready.")
